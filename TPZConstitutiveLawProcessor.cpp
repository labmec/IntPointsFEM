...
void TPZConstitutiveLawProcessor::ComputeSigmaDep(TPZFMatrix<REAL> &glob_delta_strain, TPZFMatrix<REAL> &glob_sigma, TPZFMatrix<REAL> &glob_dep) {
    ...
    tbb::parallel_for(size_t(0), size_t(fNpts), size_t(1),
                      [this, mat, & glob_delta_strain, & glob_sigma, & glob_dep](size_t &ipts) {
                          ...
                          elastic_strain(1, 0) = elastic_strain(1, 0) / 2;
                          SpectralDecomposition(&elastic_strain(0, 0), &strain_eigenvalues(0, 0),
                                                &strain_eigenvectors(0, 0));
                          ProjectSigma(&stress_eigenvalues(0, 0), &sigma_projected(0, 0), mtype, alpha, mc_phi, mc_psi,
                                       mc_cohesion, K, G, &gradient(0, 0), true);
                          TangentOperator(&gradient(0, 0), &stress_eigenvalues(0, 0), &strain_eigenvalues(0, 0),
                                          &strain_eigenvectors(0, 0), &dep(0, 0), G, lambda);
...
#ifdef USING_CUDA
void TPZConstitutiveLawProcessor::ComputeSigmaDep(TPZVecGPU<REAL> &glob_delta_strain, TPZVecGPU<REAL> &glob_sigma, TPZVecGPU<REAL> &glob_dep) {
    REAL lambda = fMaterial->GetPlasticModel().fER.Lambda();
    REAL mu =  fMaterial->GetPlasticModel().fER.Mu();
    REAL mc_phi = fMaterial->GetPlasticModel().fYC.Phi();
    REAL mc_psi = fMaterial->GetPlasticModel().fYC.Psi();
    ...
    int64_t rows = glob_delta_strain.getSize();
    glob_sigma.resize(rows);
    glob_dep.resize(3 * 3 * fNpts);

    TPZMatWithMem<TPZElastoPlasticMem> * mat = dynamic_cast<TPZMatWithMem<TPZElastoPlasticMem> *>(fMaterial);

    fCudaCalls->ComputeSigmaDep(mat->GetUpdateMem(), fNpts, glob_delta_strain.getData(), glob_sigma.getData(), glob_dep.getData(), lambda, mu, mc_phi, mc_psi, mc_cohesion, dPlasticStrain.getData(),
        dMType.getData(), dAlpha.getData(), dSigma.getData(), dStrain.getData(), dWeight.getData());
