...
//     #ifdef COMPUTE_K_GS
    std::cout << "COMPUTE_K_GPU_GS" << std::endl;          
        ...
        TPZVecGPU<REAL> d_Kc(nel_per_color * el_dofs * el_dofs);
        d_Kc.Zero();
        fCudaCalls.MatrixAssembleGS(d_Kc.getData(), first, last, d_el_color_indexes.getData(), fIntegrator.ConstitutiveLawProcessor().WeightVectorDev().getData(),
            fIntegrator.IrregularBlocksMatrix().BlocksDev().dStorage.getData(),
        ...
        // TPZVec<REAL> teste1(d_DeBip.getSize());
        // d_DeBip.get(&teste1[0], d_DeBip.getSize()); // back to CPU
        // std::cout << teste1 << std::endl;

        TPZVec<REAL> teste(d_Kc.getSize());
        d_Kc.get(&teste[0], d_Kc.getSize()); // back to CPU
...
void TPZElastoPlasticIntPointsStructMatrix::SetUpIrregularBlocksData(TPZVec<int> &element_indexes, TPZIrregularBlocksMatrix::IrregularBlocks &blocksData) {
    ...
    for (int iel = 0; iel < nblocks; ++iel) {
        ...
        // if(iel == 3) {
        //     B_el.Print("B = ", std::cout, EMathematicaInput);
        // }
        B_el.Transpose();
