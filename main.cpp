...
TPZGeoMesh *geometry_2D(int nelem_x, int nelem_y, REAL len, int ndivide) {
    ...
    if (ndivide!=0){
        ...
        int targetDim = 2;
        gel = gmesh->FindElement(x, qsi, InitialElIndex, targetDim); // Finding the element which is related to the coordinate
...
//            weight[cont_elem][i_npts] = w*std::abs(data.detjac);
            ...
            for (int i_nf=0; i_nf<nf; i_nf++){
                ...
                for (int i_dim=0; i_dim<dim_mesh; i_dim++)   AdVec[cont_elem](i_npts*dim_mesh + i_dim, i_nf) = dphix(i_dim,i_nf);
            }
        }
    ...
    // Organizando um único vetor de índices global: indexes
    ...
    // Criando a classe que irá resolver a multiplicação matricial oriunda do problema da integração numérica
    TPZSolveMatrix * SolMat = new TPZSolveMatrix(npts_tot, nf_tot, AdVec, indexes);
    
    TPZFMatrix<REAL> coef_sol = cmesh->Solution();
    
    // Result - matriz com número de linhas igual ao númeto de pontos de integ total (npts_tot) e 3 colunas:
    // col 1: Epsilon x
    // col 2: Epsilon y
    // col 3: Epsilon xy
